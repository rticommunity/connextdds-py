...
void init_class_defs(py::class_<WaitSetProperty>& cls)
{
    ...
    cls.def(py::init<int32_t, const dds::core::Duration&>(),
            py::arg("event_count"),
            py::arg("event_delay"),
    ...
    cls.def(py::self == py::self, "Compare objects for equality.");
    cls.def(py::self != py::self, "Determine if objects are unequal.");
}

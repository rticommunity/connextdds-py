...
template <> void init_class_defs(py::class_<TopicQuerySelection> &cls) {
  cls.def(py::init<const dds::topic::Filter &>(), py::arg("filter"),
          "Creates a TopicQuerySelection.")
#if rti_connext_version_gte(6, 0, 0)
      .def(py::init<const dds::topic::Filter &, TopicQuerySelectionKind>(),
           py::arg("filter"), py::arg("kind"),
           "Creates a TopicQuerySelection with a selection kind.")
#else
      .def(py::init<const dds::topic::Filter &>(), py::arg("filter"),
           "Creates a TopicQuerySelection.")
#endif
      .def_property(
          "filter",
  ...
  py::implicitly_convertible<dds::topic::Filter,
                             rti::sub::TopicQuerySelection>();
}

template <> void init_class_defs(py::class_<TopicQueryData> &cls) {
  cls.def_static("create_from_service_request",
                 &rti::sub::create_topic_query_data_from_service_request,
                 py::arg("service_request"),
                 "Creates a TopicQueryData from a ServiceRequest.")
      .def_property_readonly("selection", &TopicQueryData::selection,
                             "The data selection.")
...
template <> void init_class_defs(py::class_<TopicQuery> &cls) {
  cls.def(py::init([](PyIAnyDataReader &dr, const TopicQuerySelection &tqs) {
      ...
      .def_static(
          ...
          py::arg("reader"), "Create a TopicQuery that requests all data.")
      .def_static("find", [](PyIAnyDataReader &adr, rti::core::Guid &guid) {
...
template <> void process_inits<TopicQuery>(py::module &m, ClassInitList &l) {

#if rti_connext_version_gte(6, 0, 0)
  init_dds_safe_enum<TopicQuerySelectionKind_def>(
      m, "TopicQuerySelectionKind", [](py::object &o) {
        py::enum_<TopicQuerySelectionKind::type>(o, "Enum")
            .value("HISTORY_SNAPSHOT",
                   TopicQuerySelectionKind::type::HISTORY_SNAPSHOT,
                   "[default] Indicates that the TopicQuery may only select "
                   "samples "
                   "that were in the DataWriter cache upon reception.")
            .value(
                "CONTINUOUS", TopicQuerySelectionKind::type::CONTINUOUS,

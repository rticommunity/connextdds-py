...
void init_class_defs(py::class_<DomainParticipantConfigParams> &cls) {
  cls.def(py::init<int32_t, const std::string &, const std::string &,
                   const std::string &, const std::string &,
                   const std::string &>(),
          py::arg_v("domain_id",
                    DomainParticipantConfigParams::DOMAIN_ID_USE_XML_CONFIG,
                    "DomainParticipantConfigParams.DOMAIN_ID_USE_XML_CONFIG"),
          py::arg_v("participant_name",
...
template <>
void process_inits<DomainParticipantConfigParams>(py::module &m,
                                                  ClassInitList &l) {
  l.push_back([m]() mutable {
    return init_class<DomainParticipantConfigParams>(
        m, "DomainParticipantConfigParams");
  });
}

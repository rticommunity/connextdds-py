...
template <> void init_class_defs(py::class_<RtpsReliableReaderProtocol> &cls) {
  cls.def(py::init<>(),
      ...
      .def_property(
          ...
          "messages are sent.")
      .def_property("samples_per_app_ack",
                    (int32_t(RtpsReliableReaderProtocol::*)() const) &
...
template <>
void process_inits<RtpsReliableReaderProtocol>(py::module &m,
                                               ClassInitList &l) {
  l.push_back([m]() mutable {
    return init_class<RtpsReliableReaderProtocol>(m,
                                                  "RtpsReliableReaderProtocol");
  });
}

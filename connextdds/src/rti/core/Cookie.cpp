...
template <> void init_class_defs(py::class_<Cookie> &cls) {
  cls.def(py::init<dds::core::vector<uint8_t> &>(), "bytes")
      .def_property_readonly(
          "value", [](Cookie &c) { return std::vector<uint8_t>(c.value()); },
      ...
      .doc() = "Unique identifier for a written data sample in the form of a "
               "sequence of bytes.";
}

template <> void process_inits<Cookie>(py::module &m, ClassInitList &l) {

...
template <> void init_class_defs(py::class_<EndpointGroup> &cls) {
  cls.def(py::init<const std::string &, int32_t>(), py::arg("role_name"),
          py::arg("quorum_count"),
          "Create an EndpointGroup with the provided parameters.")
      .def_property("role_name",
  ...
  py::implicitly_convertible<std::vector<EndpointGroup>,
                             dds::core::vector<EndpointGroup>>();
}

template <> void process_inits<EndpointGroup>(py::module &m, ClassInitList &l) {

...
template<>
void pyrti::init_class_defs(py::class_<UnidimensionalCollectionTypeImpl>& cls) {
    cls
...
void pyrti::process_inits<UnidimensionalCollectionTypeImpl>(py::module& m, pyrti::ClassInitList& l) {
    l.push_back(
        [m]() mutable {
            return pyrti::init_class<UnidimensionalCollectionTypeImpl>(m, "UnidimensionalCollectionType");
        }
    ); 
}
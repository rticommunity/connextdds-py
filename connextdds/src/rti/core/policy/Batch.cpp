...
template <> void init_class_defs(py::class_<Batch> &cls) {
  cls.def(py::init<>(),
      ...
      .def_property(
          "max_samples", (int32_t(Batch::*)() const) & Batch::max_samples,
      ...
      .def_property(
          ...
          "thread safe.")
      .def_static("enabled", &Batch::Enabled,
                  "Create a Batch policy with batching enabled.")
      .def_static("disabled", &Batch::Disabled,
                  "Create a Batch policy with batching disabled.")
      .def_static("enabled_with_max_data_bytes",
                  &Batch::EnabledWithMaxDataBytes, py::arg("max_data_bytes"),
                  "Create a Batch policy with batching enabled that will flush "
                  "based on the cummulative size of serialized samples.")
      .def_static("enabled_with_max_samples", &Batch::EnabledWithMaxSamples,
                  py::arg("max_samples"),
                  "Create a Batch policy with batching enabled that will flush "
...
template <> void process_inits<Batch>(py::module &m, ClassInitList &l) {
  l.push_back([m]() mutable { return init_class<Batch>(m, "Batch"); });
}

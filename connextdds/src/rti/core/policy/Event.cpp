...
template<>
void pyrti::init_class_defs(py::class_<Event>& cls) {
    cls
        .def(
            py::init<>(),
        ...
        .def_property(
            ...
            (Event& (Event::*)(const rti::core::ThreadSettings&)) &Event::thread,
            "Event thread QoS."
        )
        .def_property(
            "initial_count",
...
template<>
void pyrti::process_inits<Event>(py::module& m, pyrti::ClassInitList& l) {
    l.push_back(
        [m]() mutable {
            return pyrti::init_class<Event>(m, "Event");
        }
    );
}

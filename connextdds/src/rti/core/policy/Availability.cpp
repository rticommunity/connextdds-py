...
template <> void init_class_defs(py::class_<Availability> &cls) {
  cls.def(py::init<>(), "Create a default Availability QoS policy.")
      ...
      .def_property(
          ...
          (Availability & (Availability::*)(const dds::core::Duration &)) &
              Availability::max_data_availability_waiting_time,
...
template <> void process_inits<Availability>(py::module &m, ClassInitList &l) {
  l.push_back(
      [m]() mutable { return init_class<Availability>(m, "Availability"); });
}

...
template <> void init_class_defs(py::class_<Property> &cls) {
  cls.def(py::init<>(), "Creates an empty Property container.")
      ...
      .def(
          ...
          py::arg("key"), py::arg("value"), py::arg("propagate") = false,
      ...
      .def("__setitem__",
           [](Property &p, const std::string &k,
              const std::pair<std::string, bool> &v) {
             return p.set(std::pair<std::string, std::string>(k, v.first),
                          v.second);
           })
...
template <> void process_inits<Property>(py::module &m, ClassInitList &l) {

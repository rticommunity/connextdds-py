...
template <> void init_class_defs(py::class_<MultiChannel> &cls) {
  cls.def(py::init<>(), "Creates the default policy.")
      .def(
          py::init<const std::vector<ChannelSettings> &, const std::string &>(),
          py::arg("channels"),
...
template <> void process_inits<MultiChannel>(py::module &m, ClassInitList &l) {
  l.push_back(
      [m]() mutable { return init_class<MultiChannel>(m, "MultiChannel"); });
}

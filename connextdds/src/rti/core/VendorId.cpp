...
template <> void init_class_defs(py::class_<VendorId> &cls) {
  cls.def(py::init<>(), "Creates unknown vendor ID.")
      ...
      .def_static("unknown", &VendorId::unknown,
                  "Create an unknown vendor ID.");
}

template <> void process_inits<VendorId>(py::module &m, ClassInitList &l) {

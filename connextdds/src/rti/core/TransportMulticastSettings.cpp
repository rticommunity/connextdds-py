...
template <> void init_class_defs(py::class_<TransportMulticastSettings> &cls) {
  cls.def(py::init<const std::vector<std::string> &, const std::string &,
                   int32_t>(),
          py::arg("transports"), py::arg("receive_address"),
          py::arg("receive_port"),
          "Creates an instance with the specified transport aliases, "
          "receive address and receive port.")
      .def_property(
          "transports",
          (std::vector<std::string>(TransportMulticastSettings::*)() const) &
              TransportMulticastSettings::transports,
          (TransportMulticastSettings &
           (TransportMulticastSettings::*)(const std::vector<std::string> &)) &
              TransportMulticastSettings::transports,
...
template <>
void process_inits<TransportMulticastSettings>(py::module &m,
                                               ClassInitList &l) {
  l.push_back([m]() mutable {
    return init_class_with_seq<TransportMulticastSettings>(
        m, "TransportMulticastSettings");
  });
}

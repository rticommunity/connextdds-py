...
template <> void init_class_defs(py::class_<Guid> &cls) {
  cls.def(py::init<>(), "Create a default Guid (equivalent to unknown Guid).")
      .def("__getitem__", [](Guid &g, uint32_t index) { return g[index]; })
      .def("__setitem__", [](Guid &g, uint32_t index,
                             uint8_t value) { return g[index] = value; })

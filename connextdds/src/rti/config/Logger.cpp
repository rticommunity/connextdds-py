...
template <> void init_class_defs(py::class_<Logger> &cls) {
  cls.def_static("get_instance", &Logger::instance,
      ...
      .def("verbosity_by_category",
           (void (Logger::*)(LogCategory, Verbosity)) &
      ...
      .def(
          "output_file_set",
          [](Logger &l, const std::string &file_prefix,
             const std::string &file_suffix, int max_bytes, int max_files) {
            l.output_file_set(file_prefix.c_str(), file_suffix.c_str(),
                              max_bytes, max_files);
          },
          py::arg("file_prefix"), py::arg("file_suffix"), py::arg("max_bytes"),
          py::arg_v("max_files", dds::core::LENGTH_UNLIMITED,
                    "LENGTH_UNLIMITED"),
          "Set the name of the file to which the logged output is "
          "redirected.");
}

template <> void process_inits<Logger>(py::module &m, ClassInitList &l) {
  init_dds_safe_enum<Verbosity_def>(m, "Verbosity", [](py::object &o) {
    py::enum_<Verbosity::type>(o, "Enum")
        .value("SILENT", Verbosity::type::SILENT,
               "No further output will be logged.")
        .value("EXCEPTION", Verbosity::type::EXCEPTION,
               ...
               "configuration.")
        .value(
            "WARNING", Verbosity::type::WARNING,

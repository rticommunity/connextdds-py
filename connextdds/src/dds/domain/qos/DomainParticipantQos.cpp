...
template <> void init_class_defs(py::class_<DomainParticipantQos> &cls) {
  cls.def(py::init<>(),
          "Create a DomainParticipantQos with the default value for each "
          "policy.")
      .def(py::init([](const PyDomainParticipant &dp) { return dp.qos(); }),
           py::arg("participant"),
  ...
  add_qos_property<DomainParticipantQos, UserData>(cls, "user_data",
                                                   "UserData");
  add_qos_property<DomainParticipantQos, EntityFactory>(cls, "entity_factory",
                                                        "EntityFactory");
  add_qos_property<DomainParticipantQos, WireProtocol>(cls, "wire_protocol",
                                                       "WireProtocol");
  add_qos_property<DomainParticipantQos, TransportBuiltin>(
      cls, "transport_builtin", "TransportBuiltin");
  add_qos_property<DomainParticipantQos, Discovery>(cls, "discovery",
                                                    "Discovery");
  add_qos_property<DomainParticipantQos, DomainParticipantResourceLimits>(
      cls, "resource_limits", "DomainParticipantResourceLimits");
  add_qos_property<DomainParticipantQos, Event>(cls, "event", "Event");
  add_qos_property<DomainParticipantQos, ReceiverPool>(cls, "receiver_pool",
                                                       "ReceiverPool");
  ...
  add_qos_property<DomainParticipantQos, DiscoveryConfig>(
      cls, "discovery_config", "DiscoveryConfig");
  ...
  add_qos_property<DomainParticipantQos, EntityName>(cls, "participant_name",
                                                     "EntityName");
  add_qos_property<DomainParticipantQos, TransportMulticastMapping>(
      cls, "transport_multicast_mapping", "TransportMulticastMapping");

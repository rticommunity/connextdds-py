...
template <> void init_class_defs(py::class_<DomainParticipantFactoryQos> &cls) {
  cls.def(py::init<>(),
  ...
  add_qos_property<DomainParticipantFactoryQos, SystemResourceLimits>(
      cls, "system_resource_limits", "SystemResourceLimits");
}

template <>
void process_inits<DomainParticipantFactoryQos>(py::module &m,
                                                ClassInitList &l) {
  l.push_back([m]() mutable {
    return init_class<DomainParticipantFactoryQos>(
        m, "DomainParticipantFactoryQos");
  });
}

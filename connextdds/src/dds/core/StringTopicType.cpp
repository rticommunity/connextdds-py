...
template <> void init_class_defs(py::class_<dds::core::StringTopicType> &cls) {
  cls.def(py::init<>(), "Initialize sample with an empty string")
      ...
      .def("__len__",
           [](const dds::core::StringTopicType &s) { return s.data().size(); })
...
template <>
void process_inits<dds::core::StringTopicType>(py::module &m,
                                               ClassInitList &l) {
  l.push_back([m, &l]() mutable {
    return init_type_class<dds::core::StringTopicType>(m, l, "StringTopicType");
  });
}

...
void init_class_defs(py::class_<PyTriggeredConditionsIterator> &cls) {
  cls.def("__next__", &PyTriggeredConditionsIterator::next,
          "Get next triggered condition.");
}

template <> void init_class_defs(py::class_<PyTriggeredConditions> &cls) {
  cls.def("__getitem__", [](PyTriggeredConditions &tc,
                            int index) { return PyCondition(tc.v()[index]); })
      .def("__contains__",
      ...
      .def("__len__", [](PyTriggeredConditions &tc) { return tc.v().size(); })
      .def(
          "__iter__",
...
template <> void init_class_defs(py::class_<WaitSet> &cls) {
  cls.def(py::init<>(), "Create a WaitSet with no conditions attached.")
      ...
      .def(
          ...
          "timeout.")
      .def(
          "wait", [](WaitSet &ws) { return PyTriggeredConditions(ws.wait()); },
      ...
      .def(
          ...
          [](WaitSet &ws, const dds::core::Duration &d) -> py::object {
            return PyAsyncioExecutor::run<PyTriggeredConditions>(
                std::function<PyTriggeredConditions()>(
                    [&ws, &d]() { return PyTriggeredConditions(ws.wait(d)); }));
          },
          py::arg("timeout"), py::keep_alive<0, 1>(), py::keep_alive<0, 2>(),
          "Wait for conditions attached to this WaitSet to trigger with a "
      ...
      .def(
          ...
          [](WaitSet &ws) {
            return PyAsyncioExecutor::run<PyTriggeredConditions>(
                std::function<PyTriggeredConditions()>(
                    [&ws]() { return PyTriggeredConditions(ws.wait()); }));
          },
          py::keep_alive<0, 1>(),
          "Wait indefinitely for conditions attached to this WaitSet to "
      ...
      .def("dispatch", (void (WaitSet::*)()) & WaitSet::dispatch,
           py::call_guard<py::gil_scoped_release>(),
           "Dispatch handlers for triggered conditions attached to this "
      ...
      .def(
          ...
          [](WaitSet &ws, const dds::core::Duration &d) {
            return PyAsyncioExecutor::run<void>(
                std::function<void()>([&ws, &d]() { ws.dispatch(d); }));
          },
          py::arg("timeout"), py::keep_alive<0, 1>(), py::keep_alive<0, 2>(),
          "Dispatch handlers for triggered conditions attached to this "
      ...
      .def(
          ...
          [](WaitSet &ws) {
            return PyAsyncioExecutor::run<void>(
                std::function<void()>([&ws]() { ws.dispatch(); }));
          },
          py::keep_alive<0, 1>(),
          "Dispatch handlers for triggered conditions attached to this "
      ...
      .def(
          "__iadd__",
          [](WaitSet &ws, PyICondition &c) { return ws += c.get_condition(); },
          py::is_operator(), "Attach a condition to a WaitSet.")
      .def(
          "__isub__",
          [](WaitSet &ws, PyICondition &c) { return ws -= c.get_condition(); },
          py::is_operator(), "Detach a condition from a WaitSet.")
      .def(
          "attach_condition",
          [](WaitSet &ws, PyICondition &c) {
            ws.attach_condition(c.get_condition());
          },
          py::arg("condition"), "Attach a condition to the WaitSet.")
      .def(
          "detach_condition",
          [](WaitSet &ws, PyICondition &c) {
            return ws.detach_condition(c.get_condition());
          },
          py::arg("condition"), "Detach a condition from the WaitSet.")
      .def_property(
          "conditions",
          [](WaitSet &ws) {
      ...
      .def(
          "detach_all", [](WaitSet &ws) { ws->detach_all(); },
          "Detach all conditions from the WaitSet.")
      .def_property(
          "property", [](WaitSet &ws) { return ws->property(); },
...
template <> void process_inits<WaitSet>(py::module &m, ClassInitList &l) {

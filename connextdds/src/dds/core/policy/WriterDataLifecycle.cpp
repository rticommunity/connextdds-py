...
template <> void init_class_defs(py::class_<WriterDataLifecycle> &cls) {
  cls.def(py::init<>(), "Creates the default policy.")
      .def(py::init<bool>(), py::arg("autodispose"),
      ...
      .def_property(
          ...
          "\n\n"
          "This property's getter returns a deep copy.")
      .def_static("auto_dispose_unregistered_instances",
                  &WriterDataLifecycle::AutoDisposeUnregisteredInstances,
                  "Creates WriterDataLifecycle(true).")
      .def_static("manually_dispose_unregistered_instances",

...
template <> void init_class_defs(py::class_<DataTag> &cls) {
  cls.def(py::init<>(), "Creates a policy with an empty sequence of tags.")
      ...
      .def(
          ...
          py::arg("entry_map"), "Adds or assigns tags from a dictionary.")
      .def(
          "set",
          [](DataTag &dt, std::pair<std::string, std::string> entry) {
            dt.set(entry);
          },
          py::arg("entry"), "Adds or assigns a tag from a pair of strings.")
      .def(
          "set",
      ...
      .def("exists", &DataTag::exists, py::arg("key"),
           "Returns true if a tag exists.")
      .def("get", &DataTag::get, py::arg("key"),
           "Returns the value of a tag identified by a key if it exists.")
      .def("try_get", &DataTag::try_get, py::arg("key"),
           "Returns the value of a tag identified by a key if it exists.")
      .def("remove", &DataTag::remove, py::arg("key"),
           "Removes the tag identified by a key.")
      .def("size", &DataTag::size, "Returns the number of tags.")
      .def("get_all", &DataTag::get_all, "Retrieves a copy of all the entries.")
...
template <> void process_inits<DataTag>(py::module &m, ClassInitList &l) {

...
template <> void init_class_defs(py::class_<TopicData> &cls) {
...
template <> void process_inits<TopicData>(py::module &m, ClassInitList &l) {
  l.push_back([m]() mutable { return init_class<TopicData>(m, "TopicData"); });
}

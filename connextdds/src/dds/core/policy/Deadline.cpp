...
template <> void init_class_defs(py::class_<Deadline> &cls) {
  cls.def(py::init<>(),
          "Creates the default deadline, with an infinite period.")
      .def(py::init<const dds::core::Duration &>(), py::arg("period"),
...
template <> void process_inits<Deadline>(py::module &m, ClassInitList &l) {

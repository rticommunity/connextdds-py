...
template <> void init_class_defs(py::class_<Presentation> &cls) {
  cls.def(py::init<>(), "Create an instance with the default value.")
      ...
      .def_property("ordered_access",
                    ...
                    "access_scope.")
      .def_static("group_access_scope", &Presentation::GroupAccessScope,
                  py::arg("coherent"), py::arg("ordered"),
                  "Creates a Presentation instance with group access scope.")
      .def_static("instance_access_scope", &Presentation::InstanceAccessScope,
                  py::arg("coherent"), py::arg("ordered"),
                  "Creates a Presentation instance with instance access scope.")
      .def_static("topic_access_scope", &Presentation::TopicAccessScope,
                  py::arg("coherent"), py::arg("ordered"),
...
static void init_dds_presentation_access_scope_kind(py::module &m) {
  init_dds_safe_enum<PresentationAccessScopeKind_def>(
      m, "PresentationAccessScopeKind", [](py::object &o) {
        py::enum_<PresentationAccessScopeKind::type>(o, "Enum")
            .value("INSTANCE", PresentationAccessScopeKind::type::INSTANCE,
                   "[default] Scope spans only a single instance."
                   "\n\n"
                   "Indicates that changes to one instance need not be "
                   "coherent nor "
                   "ordered with respect to changes to any other instance. In "
                   "other "
                   "words, order and coherent changes apply to each instance "
                   "separately.")
            .value(
                "TOPIC", PresentationAccessScopeKind::type::TOPIC,
                "Scope spans to all instances within the same DataWriter (or "
                "DataReader), but not across instances in different DataWriter "
                "(or DataReader).")
            .value("GROUP", PresentationAccessScopeKind::type::GROUP,
                   "Scope spans to all instances belonging to DataWriter (or "
                   "DataReader) entities within the same Publisher (or "
                   "Subscriber).")
            .value("HIGHEST_OFFERED",
                   PresentationAccessScopeKind::type::HIGHEST_OFFERED,
                   "This value only applies to a Subscriber. The Subscriber "
                   "will use "
...
template <> void process_inits<Presentation>(py::module &m, ClassInitList &l) {

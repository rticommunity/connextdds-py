...
template<>
void pyrti::init_class_defs(py::class_<Duration>& cls) {
    cls
        .def(
            py::init<>(),
...
template<>
void pyrti::process_inits<Duration>(py::module& m, pyrti::ClassInitList& l) {
    l.push_back(
        [m]() mutable {
            return pyrti::init_class<Duration>(m, "Duration");
        }
    );   
}
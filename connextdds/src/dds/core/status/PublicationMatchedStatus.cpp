...
template <> void init_class_defs(py::class_<PublicationMatchedStatus> &cls) {
  cls.def_property_readonly(
         "total_count", &PublicationMatchedStatus::total_count,
         "Total count of times the DataWriter matched with a DataReader.")
      .def_property_readonly(
          "total_count_change", &PublicationMatchedStatus::total_count_change,
          "The delta in the total count since the last time the listener "
          "fired or the status was read.")
      .def_property_readonly(
          "current_count", &PublicationMatchedStatus::current_count,

...
void init_class_defs(
  ...
  cls.def(py::init<const std::string &, const DynamicType &>(), py::arg("name"),
          py::arg("discriminator_type"), "Creates an empty Union.")
      .def(py::init<const std::string &, const DynamicType &,
                    const std::vector<UnionMember> &>(),
           py::arg("name"), py::arg("discriminator_type"), py::arg("members"),
           "Creates a Union with the specified UnionMembers.")
      .def(py::init(
               [](DynamicType &dt) { return static_cast<UnionType &>(dt); }),
           py::arg("type"), "Cast a DynamicType to a UnionType.")
      .def_property_readonly("descriminator", &UnionType::discriminator,
      ...
      .def(
          ...
          py::arg("label"), "Gets the index of the member selected by a label.")
      .def("find_member_by_id", &UnionType::find_member_by_id, py::arg("id"),
...
template <> void process_inits<UnionType>(py::module &m, ClassInitList &l) {

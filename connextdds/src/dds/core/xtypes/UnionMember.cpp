...
template<>
void pyrti::init_class_defs(py::class_<UnionMember>& cls) {
    cls
        .def(
...
template<>
void pyrti::process_inits<UnionMember>(py::module& m, pyrti::ClassInitList& l) {
    l.push_back(
        [m]() mutable {
            return pyrti::init_class<UnionMember>(m, "UnionMember");
        }
    );  
}

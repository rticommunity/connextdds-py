...
template <typename T>
static TypeKind::inner_enum
resolve_type_kind(DynamicData &dd, TypeKind::inner_enum kind, const T &key) {
...
static int calculate_multidimensional_offset(DynamicType &type,
                                             py::tuple &index) {
...
template <typename T>
static py::object get_collection_member(DynamicData &dd,
...
template <typename T>
static py::object get_member(DynamicData &dd, TypeKind::inner_enum kind,
                             const T &key) {
  kind = resolve_type_kind(dd, kind, key);
  switch (kind) {
  case TypeKind::BOOLEAN_TYPE:
...
template <typename T>
static void set_collection_member(DynamicData &dd, TypeKind::inner_enum kind,
                                  const T &key, py::object &values) {
  kind = resolve_type_kind(dd, kind, key);
  switch (kind) {
  case TypeKind::BOOLEAN_TYPE: {
  ...
  case TypeKind::INT_64_TYPE:
    dd.set_values<rti::core::int64>(
        key, py::cast<std::vector<rti::core::int64>>(values));
    break;
  case TypeKind::UINT_64_TYPE:
    dd.set_values<rti::core::uint64>(
        key, py::cast<std::vector<rti::core::uint64>>(values));
    break;
  case TypeKind::FLOAT_32_TYPE:
...
template <typename T>
static void set_member(DynamicData &dd, TypeKind::inner_enum kind, const T &key,
                       py::object value) {
  if (value.is_none()) {
  ...
  case TypeKind::FLOAT_64_TYPE:
    ...
    break;
  case TypeKind::FLOAT_128_TYPE:
    dd.value<rti::core::LongDouble>(key,
...
static bool test_index(const DynamicData &dd, TypeKind::inner_enum kind,
                       int index, const py::object &obj) {
  switch (kind) {
  case TypeKind::BOOLEAN_TYPE:
...
void update_dynamicdata_object(DynamicData &dd, py::dict &dict) {
  for (auto kv : dict) {
    ...
    set_member(dd, mi.member_kind().underlying(), key,
               py::cast<py::object>(kv.second));
  }
}
...
static py::object get_values(DynamicData &dd, const T &key) {
  ...
  auto mi = dd.member_info(key);
  auto kind =
      resolve_type_kind(dd, mi.member_kind().underlying(), mi.member_index());
...
template <typename T>
static void set_values(DynamicData &dd, const std::string &key,
                       py::object values) {
  TypeKind::type field_type;
  if (dd.member_exists_in_type(key)) {
    field_type = resolve_type_kind(
        dd, dd.member_info(key).member_kind().underlying(), key);
  } else {
    rti::core::xtypes::LoanedDynamicData loan = dd.loan_value(key);
    DynamicData &member = loan.get();
...
public:
  ...
  size_t len() { return this->_dd.member_count(); }
...
public:
  ...
  std::pair<std::string, py::object> next() {
    ...
    auto retval = std::pair<std::string, py::object>(
        field_name,
        get_member(this->_dd,
                   this->_dd.member_info(field_name).member_kind().underlying(),
                   field_name));
    this->_index += this->_step;
    return retval;
  }
...
public:
  ...
  bool contains(const std::pair<std::string, py::object> &item) {
    return _dd.member_exists_in_type(item.first) &&
           test_index(
               this->_dd,
               this->_dd.member_info(item.first).member_kind().underlying(),
               this->_dd.member_index(item.first), item.second);
  }
...
void init_dds_typed_topic_template(
    py::class_<PyTopic<DynamicData>, PyITopicDescription<DynamicData>,
               PyIAnyTopic> &cls) {
  init_dds_typed_topic_base_template(cls);
  cls.def(py::init([](PyDomainParticipant &dp, const ::std::string &name,
                      const dds::core::xtypes::DynamicType &type) {
            PyTopic<dds::core::xtypes::DynamicData> t(dp, name, type);
            PyDynamicTypeMap::add(t.type_name(), type);
            return t;
          }),
          py::arg("participant"), py::arg("topic_name"), py::arg("topic_type"),
          py::call_guard<py::gil_scoped_release>(),
          "Create a Topic with the given type.")
      .def(
          py::init([](PyDomainParticipant &dp, const ::std::string &name,
                      const dds::core::xtypes::DynamicType &type,
                      const dds::topic::qos::TopicQos &qos,
                      PyTopicListener<dds::core::xtypes::DynamicData> *listener,
                      const dds::core::status::StatusMask &mask) {
            PyTopic<dds::core::xtypes::DynamicData> t(dp, name, type, qos,
                                                      listener, mask);
            PyDynamicTypeMap::add(t.type_name(), type);
            return t;
          }),
          py::arg("participant"), py::arg("topic_name"), py::arg("topic_type"),
          py::arg("qos"), py::arg("listener") = py::none(),
...
void init_dds_typed_datawriter_template(
  ...
  cls.def(
      ...
      .def(
          ...
          py::arg("sample_data"), py::keep_alive<0, 1>(),
          py::keep_alive<0, 2>(),
          "Create a DynamicData object and write it with the given "
      ...
      .def(
          ...
          [](PyDataWriter<dds::core::xtypes::DynamicData> &dw,
            ...
            dds::core::xtypes::DynamicData d(
                PyDynamicTypeMap::get(dw->type_name()));
            dds::topic::TopicInstance<dds::core::xtypes::DynamicData> ti(handle,
                                                                         d);
...
void init_dds_typed_datareader_template(
  ...
  cls.def(
      ...
      .def(
          ...
          [](PyDataReader<dds::core::xtypes::DynamicData> &dr) {
            dds::core::optional<
                dds::sub::Sample<dds::core::xtypes::DynamicData>>
                retval;
            ...
            if (dr->read(data, info)) {
              retval =
                  dds::sub::Sample<dds::core::xtypes::DynamicData>(data, info);
            }
            return retval;
          },
...
void init_dds_typed_sample_template(
  ...
  cls.def(py::init([](rti::sub::LoanedSample<DynamicData> &loaned_sample) {
            return dds::sub::Sample<DynamicData>(loaned_sample.data(),
                                                 loaned_sample.info());
          }),
          py::arg("loaned_sample"), "Copy constructor.");
}

template <typename T>
void add_field_type(py::class_<DynamicData> &cls, const std::string &type_str,
...
template <typename T>
void add_field_type_collection(py::class_<DynamicData> &cls,
  ...
  cls.def(("get_" + type_str + "_values").c_str(),
      ...
      .def(
          ...
          (void (DynamicData::*)(const std::string &, const std::vector<T> &)) &
...
template <> void init_class_defs(py::class_<DynamicData> &dd_class) {
  dd_class.def(py::init<const DynamicType &>())
      .def(py::init<const DynamicType &,
  ...
  py::class_<PyDynamicDataFieldsIterator> fields_iterator(dd_class,
                                                          "FieldsIterator");
  py::class_<PyDynamicDataIndexIterator> index_iterator(dd_class,
                                                        "IndexIterator");
  py::class_<PyDynamicDataItemsView> items_view(dd_class, "ItemsView");
  py::class_<PyDynamicDataItemsIterator> items_iterator(dd_class,
                                                        "ItemsIterator");
  ...
  add_field_type_collection<uint16_t>(dd_class, "uint16",
                                      "16-bit unsigned int");
  add_field_type_collection<DDS_Long>(dd_class, "int32", "32-bit signed int");
  add_field_type_collection<DDS_UnsignedLong>(dd_class, "uint32",
                                              "32-bit unsigned int");
  add_field_type_collection<DDS_LongLong>(dd_class, "int64",
                                          "64-bit signed int");
  add_field_type_collection<DDS_UnsignedLongLong>(dd_class, "uint64",
                                                  "64-bit unsigned int");
  add_field_type_collection<float>(dd_class, "float32",
                                   "32-bit floating point");
  add_field_type_collection<double>(dd_class, "float64",
  ...
  dd_class
      .def(
          ...
          py::arg("name"), "Get a wchar value by field name.")
      .def(
          "get_wchar",
          ...
          py::arg("index"), "Get a wchar value by field index.")
      .def(
          "set_wchar",
          ...
          py::arg("name"), py::arg("value"), "Get a wchar value by field name.")
      .def(
          "set_wchar",
      ...
      .def(
          ...
          py::arg("name"), "Get a wstring value by field name.")
      .def(
          "get_wstring",
          [](const DynamicData &data, uint32_t index) {
            if (data.type_kind().underlying() != TypeKind::UNION_TYPE) {
          ...
          py::arg("index"), "Get a wstring value by field index.")
      .def(
          "set_wstring",
          ...
          py::arg("name"), py::arg("value"), "Set a wstring by field name.")
      .def(
          "set_wstring",
      ...
      .def(
          ...
          py::arg("name"), "Get a list of complex values by field name.")
      .def(
          "get_complex_values",
          ...
          py::arg("index"), "Get a list of complex fields by index.")
      .def(
          "set_complex_values",
          [](DynamicData &data, const std::string &name,
             std::vector<DynamicData> &values) {
      ...
      .def("loan_value",
           (rti::core::xtypes::LoanedDynamicData &
            (DynamicData::*)(rti::core::xtypes::LoanedDynamicData &,
                             const std::string &)) &
               DynamicData::loan_value,
           py::arg("data"), py::arg("name"), py::keep_alive<0, 1>(),
           "Gets a view of a complex member.")
      .def(
          "loan_value",
          [](DynamicData &data, rti::core::xtypes::LoanedDynamicData &loan,
             uint32_t index) -> rti::core::xtypes::LoanedDynamicData & {
            if (data.type_kind().underlying() != TypeKind::UNION_TYPE) {
              index += 1;
            }
            return data.loan_value(loan, index);
          },
          py::arg("data"), py::arg("index"), py::keep_alive<0, 1>(),
          "Gets a view of a complex member.")
      .def_property_readonly("discriminator_value",
                             &DynamicData::discriminator_value,
                             "Obtains the value of the union discriminator "
                             "(valid for UnionType only).")
      .def("clear_all_members", &DynamicData::clear_all_members,
           "Clear the contents of all data members of this object, including "
           "key members.")
      .def(
      ...
      .def(
          ...
          "Clear the contents of a single data member of this object.")
#endif
      .def("set_buffer", &DynamicData::set_buffer, py::arg("storage"),
           py::arg("size"), "Associate a buffer with this dynamic data object.")
      .def_property_readonly(
          "estimated_max_buffer_size", &DynamicData::estimated_max_buffer_size,
      ...
      .def_property_readonly("type_kind", &DynamicData::type_kind,
                             "Gets this data type kind of this DynamicData.")
      .def_property_readonly("member_count", &DynamicData::member_count,
      ...
      .def(
          ...
          py::arg("index"), "Returns info about a member.")
      .def("member_index", &DynamicData::member_index, py::arg("name"),
           "Translates from member name to member index.")
      .def("is_member_key",
           (bool (DynamicData::*)(const std::string &) const) &
               DynamicData::is_member_key,
           py::arg("name"), "Returns whether a member is a key.")
      .def(
          "is_member_key",
          [](const DynamicData &data, uint32_t index) {
            return data.is_member_key(index + 1);
          },
          py::arg("index"), "Returns whether a member is a key.")
      .def(
          "__str__",
      ...
      .def(
          ...
          [](DynamicData &dd, py::object &value) {
            ...
            set_member(dd, collection_type.content_type().kind().underlying(),
                       dd.member_count() + 1, value);
          },
          py::arg("value"), "Append a value to a sequence.")
      .def(
          "extend",
          [](DynamicData &dd, py::iterable &values) {
            ...
            for (auto value : values) {
              set_member(dd, elem_kind, dd.member_count() + 1,
                         py::cast<py::object>(value));
            }
          },
          py::arg("values"), "Extend a sequence.")
      .def("count",
           [](const DynamicData &dd, const py::object &obj) {
             if (dd.type_kind().underlying() != TypeKind::ARRAY_TYPE &&
               ...
               throw py::type_error(
                   "This DynamicData type does not support this operation");
             }
      ...
      .def("get_value", &get_value<std::string>, py::arg("field_path"),
           "Automatically resolve type and return value for a field.")
      .def("get_values", &get_values<std::string>, py::arg("field_path"),
           "Automatically resolve type and return collection for a field.")
      .def("set_value", &set_value<std::string>, py::arg("field_path"),
           py::arg("value"),
           "Automatically resolve type and set value for a field.")
      .def("set_values", &set_values<std::string>, py::arg("field_path"),
           py::arg("values"),
      ...
      .def("__getitem__",
           [](DynamicData &dd, size_t index) -> py::object {
             ...
             case TypeKind::SEQUENCE_TYPE: {
               auto collection_type = static_cast<const CollectionType &>(type);
               return get_member(
                   dd, collection_type.content_type().kind().underlying(),
                   index);
             }
             default:
               throw py::type_error(
                   "This DynamicData type does not support index access.");
             }
           })
      .def("__setitem__",
      ...
      .def("__getitem__",
           [](DynamicData &dd, py::slice slice) {
             ...
             if (!slice.compute(dd.member_count(), &start, &stop, &step,
                                &slicelength))
  ...
  items_view.def(py::init<DynamicData &>())
      .def("__iter__", &PyDynamicDataItemsView::iter, py::keep_alive<0, 1>())
...
template <> void process_inits<DynamicData>(py::module &m, ClassInitList &l) {

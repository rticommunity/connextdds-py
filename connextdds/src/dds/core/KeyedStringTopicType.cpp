...
void init_class_defs(py::class_<dds::core::KeyedStringTopicType> &cls) {
  cls.def(py::init<>(), "Initialize sample with an two empty strings.")
      .def(py::init<const std::string &, const std::string &>(), py::arg("key"),
           py::arg("value"), "Initialize sample with provided key and value.")
      .def(py::init([](const std::pair<std::string, std::string> &p) {
             return dds::core::KeyedStringTopicType(p.first, p.second);
           }),
           py::arg("pair"), "Initialize sample with provided key/value pair.")
      .def_property(
          "key",
  ...
  py::implicitly_convertible<std::pair<std::string, std::string>,
                             dds::core::KeyedStringTopicType>();
}

template <>
void process_inits<dds::core::KeyedStringTopicType>(py::module &m,
                                                    ClassInitList &l) {
  l.push_back([m, &l]() mutable {
    return init_type_class<dds::core::KeyedStringTopicType>(
        m, l, "KeyedStringTopicType");
  });
}

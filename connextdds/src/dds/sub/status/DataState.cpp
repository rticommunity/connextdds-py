...
template <> void init_class_defs(py::class_<SampleState> &cls) {
  cls.def_static("read", &SampleState::read, "Creates a read SampleState.")
      .def_static("not_read", &SampleState::not_read,
...
template <> void init_class_defs(py::class_<ViewState> &cls) {
  cls.def_static("new_view", &ViewState::new_view,
                 "Creates a new_view ViewState object.")
      .def_static("not_new_view", &ViewState::not_new_view,
                  "Creates a not_new_view ViewState object.")
      .def_static("any", &ViewState::any, "Creates an any ViewState object.");
}

template <> void init_class_defs(py::class_<InstanceState> &cls) {
  cls.def_static("alive", &InstanceState::alive,
                 "Creates a alive InstanceState object.")
      .def_static("not_alive_disposed", &InstanceState::not_alive_disposed,
                  "Creates a not_alive_disposed InstanceState object.")
      .def_static("not_alive_no_writers", &InstanceState::not_alive_no_writers,
                  "Creates a not_alive_no_writers InstanceState object.")
      .def_static("not_alive_mask", &InstanceState::not_alive_mask,
                  "Creates an InstanceState object representing any not alive "
                  "states (not_alive_disposed | not_alive_no_writers")
      .def_static(
          "any", &InstanceState::any,
          "Creates an InstanceState object representing any instance states "
          "(not_alive_disposed | not_alive_no_writers | alive");
}

template <> void init_class_defs(py::class_<DataState> &cls) {
  cls.def(py::init<>(),
          "Create a DataState with InstanceState.any(), ViewState.any(), "
          "and SampleState.any()")
      .def(py::init<const SampleState &>(), py::arg("sample_state"),
           "Create a DataState with InstanceState.any(), ViewState.any(), "
           "and the provided SampleState.")
      .def(py::init<const ViewState &>(), py::arg("view_state"),
           "Create a DataState with InstanceState.any(), SampleState.any(), "
           "and the provided ViewState.")
      .def(py::init<const InstanceState &>(), py::arg("instance_state"),
           "Create a DataState with ViewState.any(), SampleState.any(), "
           "and the provided InstanceState.")
      .def(py::init<const SampleState &, const ViewState &,
                    const InstanceState &>(),
           py::arg("sample_state"), py::arg("view_state"),
           py::arg("instance_state"),
           "Create a DataState with the provided SampleState, ViewState, and "
           "InstanceState.")
      .def(py::self == py::self, "Compare DataState objects for equality.")
      .def(py::self != py::self, "Compare DataState objects for inequality.")
      .def("__lshift__",
           (DataState & (DataState::*)(const SampleState &)) &
               DataState::operator<<,
           py::is_operator(), "Set the SampleState for the DataState.")
      .def("__lshift__",
           (DataState & (DataState::*)(const ViewState &)) &
               DataState::operator<<,
           py::is_operator(), "Set the ViewState for the DataState.")
      .def("__lshift__",
           (DataState & (DataState::*)(const InstanceState &)) &
               DataState::operator<<,
           py::is_operator(), "Set the InstanceState for the DataState.")
      .def_property(
          "sample_state",

...
void init_class_defs(py::class_<PyAnyDataReader, PyIAnyDataReader> &cls) {
  cls.def(py::init<const PyIAnyDataReader &>(), py::arg("reader"),
...
template <> void init_class_defs(py::class_<PyIAnyDataReader> &cls) {
  cls.def_property("qos",
      ...
      .def("retain", &PyIAnyDataReader::py_retain, "Retain this DataReader.")
      .def(
          "__eq__",
...
template <> void process_inits<AnyDataReader>(py::module &m, ClassInitList &l) {

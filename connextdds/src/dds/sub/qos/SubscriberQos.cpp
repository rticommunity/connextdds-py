...
template <> void init_class_defs(py::class_<SubscriberQos> &cls) {
  cls.def(py::init<>(),
  ...
  add_qos_property<SubscriberQos, Presentation>(cls, "presentation",
                                                "Presentation");
  ...
  add_qos_property<SubscriberQos, EntityFactory>(cls, "entity_factory",
                                                 "EntityFactory");
  add_qos_property<SubscriberQos, ExclusiveArea>(cls, "exclusive_area",
                                                 "ExclusiveArea");
  add_qos_property<SubscriberQos, EntityName>(cls, "entity_name", "EntityName");
}

template <> void process_inits<SubscriberQos>(py::module &m, ClassInitList &l) {
  l.push_back(
      [m]() mutable { return init_class<SubscriberQos>(m, "SubscriberQos"); });
}

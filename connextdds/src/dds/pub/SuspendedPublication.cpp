...
template <> void init_class_defs(py::class_<SuspendedPublication> &cls) {
  cls.def(py::init<const PyPublisher &>(), py::arg("publisher"),
          "Suspends the publications of the publisher.")
      .def("resume", &SuspendedPublication::resume,
           "Indicates that the application has completed these changes.")
      .def(
          "__enter__", [](SuspendedPublication &s) { return s; },
          "Context manage the SuspendedPublication.")
      .def("__exit__", [](SuspendedPublication &s, py::object, py::object,
                          py::object) { s.resume(); });
}

...
template <> void init_class_defs(py::class_<PublisherQos> &cls) {
  cls.def(py::init<>(),
  ...
  add_qos_property<PublisherQos, Presentation>(cls, "presentation",
                                               "Presentation");
  ...
  add_qos_property<PublisherQos, EntityFactory>(cls, "entity_factory",
                                                "EntityFactory");
  add_qos_property<PublisherQos, AsynchronousPublisher>(
      cls, "asynchronous_publisher", "AsynchronousPublisher");
  add_qos_property<PublisherQos, ExclusiveArea>(cls, "exclusive_area",
                                                "ExclusiveArea");
  add_qos_property<PublisherQos, EntityName>(cls, "entity_name", "EntityName");
}

template <> void process_inits<PublisherQos>(py::module &m, ClassInitList &l) {
  l.push_back(
      [m]() mutable { return init_class<PublisherQos>(m, "PublisherQos"); });
}

...
PyPublisher::PyPublisher(const PyDomainParticipant &p,
...
template <> void init_class_defs(py::class_<PyPublisher, PyIEntity> &cls) {
  cls

      .def(py::init<const PyDomainParticipant &>(), py::arg("participant"),
           py::call_guard<py::gil_scoped_release>(), "Create a publisher.")
      .def(
          py::init([](const PyDomainParticipant &dp, const qos::PublisherQos &q,
          ...
          "Create a Publisher with the desired QoS policies and specified "
          "listener")
      ...
      .def(
          ...
          py::is_operator(), "Set the PublisherQos.")
      .def(
          "__rshift__",
          [](PyPublisher &pub, qos::PublisherQos &qos) {
            return PyPublisher(pub >> qos);
          },
          py::is_operator(), "Copy the PublisherQos.")
      .def_property(
      ...
      .def(
          "bind_listener",
          [](PyPublisher &pub, dds::core::optional<PyPublisherListener *> l,
             const dds::core::status::StatusMask &m) {
            auto listener = has_value(l) ? get_value(l) : nullptr;
            if (nullptr != listener) {
      ...
      .def("wait_for_acknowledgments", &PyPublisher::wait_for_acknowledgments,
           py::arg("max_wait"),
      ...
      .def(
          ...
          py::arg("name"), "Find a DataWriter in this Publisher by its name.")
      .def(
          ...
          py::arg("topic_name"),
          "Find a DataWriter in this Publisher by its topic name. If more than "
          "one exists for this Topic, the first one found is returned.")
      .def(
          "find_datawriters",
          [](const PyPublisher &pub) {
...
template <> void process_inits<Publisher>(py::module &m, ClassInitList &l) {

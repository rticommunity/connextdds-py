...
void init_class_defs(py::class_<TopicQos>& cls) {
    ...
    add_qos_property<TopicQos, LatencyBudget>(cls, "latency_budget", "LatencyBudget");
    ...
    add_qos_property<TopicQos, DestinationOrder>(cls, "destination_order", "DestinationOrder");
    add_qos_property<TopicQos, History>(cls, "history", "History");
    add_qos_property<TopicQos, ResourceLimits>(cls, "resource_limits", "ResourceLimits");
...
void process_inits<TopicQos>(py::module& m, ClassInitList& l) {
    l.push_back(
        [m]() mutable {
            return init_class<TopicQos>(m, "TopicQos");
        }
    ); 
}

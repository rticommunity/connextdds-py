...
template <> void init_class_defs(py::class_<TopicQos> &cls) {
  cls.def(py::init<>(),
          "Create a TopicQos with the default value for each policy.")
      .def(py::init([](const PyIAnyTopic &topic) { return topic.py_qos(); }),
           py::arg("topic"),
  ...
  add_qos_property<TopicQos, DurabilityService>(cls, "durability_service",
                                                "DurabilityService");
  add_qos_property<TopicQos, Deadline>(cls, "deadline", "Deadline");
  add_qos_property<TopicQos, LatencyBudget>(cls, "latency_budget",
                                            "LatencyBudget");
  ...
  add_qos_property<TopicQos, DestinationOrder>(cls, "destination_order",
                                               "DestinationOrder");
  add_qos_property<TopicQos, History>(cls, "history", "History");
  add_qos_property<TopicQos, ResourceLimits>(cls, "resource_limits",
                                             "ResourceLimits");
  add_qos_property<TopicQos, TransportPriority>(cls, "transport_priority",
                                                "TransportPriority");
  ...
  add_qos_property<TopicQos, DataRepresentation>(cls, "data_representation",
                                                 "DataRepresentation");
#endif
}

template <> void process_inits<TopicQos>(py::module &m, ClassInitList &l) {

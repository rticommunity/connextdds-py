...
template <> void init_class_defs(py::class_<Filter> &cls) {
  cls.def(py::init<const std::string &>(), py::arg("expression"),
          "Create a Filter with the specified expression.")
      .def(py::init<const std::string &, const std::vector<std::string> &>(),
           py::arg("expression"), py::arg("parameters"),
           "Create a Filter with the specified expression and parameters.")
      .def_property_readonly("expression", &Filter::expression,
                             "Get the Filter's expression.")
      .def(
          "__iter__",
          [](Filter &f) { return py::make_iterator(f.begin(), f.end()); },
          py::keep_alive<0, 1>())
      .def(
          "set_parameters",
      ...
      .def("add_parameters", &Filter::add_parameter, py::arg("parameter"),
           "Append a parameter to the Filter.")
      .def_property_readonly("parameter_count", &Filter::parameters_length,
                             "Get the number of parameters.")
      .def_property(
          "name", [](const Filter &f) { return f->name(); },
      ...
      .def_property_readonly_static(
          ...
          [](py::object &) { return rti::topic::stringmatch_filter_name(); },
          "The name of the string match filter.");

  py::implicitly_convertible<std::string, Filter>();
}

template <> void process_inits<Filter>(py::module &m, ClassInitList &l) {

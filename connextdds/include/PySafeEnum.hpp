...
template <typename T>
py::class_<safe_enum<T>>
init_dds_safe_enum(py::module &m, const std::string &name,
                   std::function<void(py::object &)> enum_init) {
  py::class_<safe_enum<T>> cls(m, name.c_str());
  enum_init(cls);
  cls.def(py::init<>(), "Initializes enum to 0.")
      .def(py::init<typename safe_enum<T>::inner_enum>(), "Copy constructor.")
      .def_property_readonly("underlying", &safe_enum<T>::underlying,
      ...
      .def(
          ...
          "String conversion.")
      .def(
          "__int__", [](const safe_enum<T> &k) { return k.underlying(); },
          "Int conversion.");

...
template <typename T>
static void init_dds_vector_defs(py::class_<dds::core::vector<T>> &cls,
                                 const std::string &name) {
  cls.def(py::init<>(), ("Create an empty " + name).c_str())
      .def(py::init<size_t>(), py::arg("size"),
           ("Create a " + name + " with a preallocated size.").c_str())
      .def(py::init<const dds::core::vector<T> &>(), py::arg("vector"),
      ...
      .def(py::init<std::vector<T>>(), py::arg("list"),
           ("Create a " + name + " from a list of values.").c_str())
      .def("__len__", &dds::core::vector<T>::size,
      ...
      .def(
          "__setitem__",
          [](dds::core::vector<T> &v, ssize_t i, const T &b) { v[i] = b; },
      ...
      .def(
          ...
          py::keep_alive<0, 1>(), "Iterate over the contents of the vector.")
      .def("clear", &dds::core::vector<T>::clear,
           ("Resize " + name + " to 0.").c_str())
      .def("resize",
           (void (dds::core::vector<T>::*)(size_t)) &
               dds::core::vector<T>::resize,
           py::arg("size"), ("Resize " + name + ".").c_str())
      .def(py::self == py::self,
...
template <typename T>
static void init_dds_vector_buffer_defs(py::class_<dds::core::vector<T>> &cls,
                                        const std::string &name) {
  cls.def(py::init([](py::buffer const b) {
      ...
      .def_buffer([](dds::core::vector<T> &v) {
        return py::buffer_info(&*v.begin(), {static_cast<ssize_t>(v.size())},
                               {1});
      });
...
template <typename T>
static void init_dds_vector_nonbuffer_class(py::module &m,
                                            const std::string &cls_name,
                                            ClassInitList &l) {
  l.push_back([m, cls_name]() mutable {
    py::class_<dds::core::vector<T>> cls(m, cls_name.c_str());
    return (
        [cls, cls_name]() mutable { init_dds_vector_defs<T>(cls, cls_name); });
  });
}

template <typename T>
static void init_dds_vector_buffer_class(py::module &m,
                                         const std::string &cls_name,
                                         ClassInitList &l) {
  l.push_back([m, cls_name]() mutable {
    py::class_<dds::core::vector<T>> cls(m, cls_name.c_str(),

...
template <typename T>
bool operator==(const dds::topic::TopicInstance<T> &left,
                const dds::topic::TopicInstance<T> &right) {
  return (left.sample() == right.sample()) && (left.handle() == right.handle());
}
} // namespace topic
} // namespace dds
...
void init_dds_typed_topic_instance_base_template(
  ...
  cls.def(py::init<const dds::core::InstanceHandle &, const T &>(),
          py::arg("handle"), py::arg("sample"),
          "Create a TopicInstance with an InstanceHandle and a data sample.")
      .def_property(
          "handle",
          (dds::core::InstanceHandle(dds::topic::TopicInstance<T>::*)() const) &
...
void init_dds_typed_topic_instance_template(
  ...
  cls.def(py::init<>(), "Create a null TopicInstance.")
      .def(py::init<const dds::core::InstanceHandle &>(), py::arg("handle"),

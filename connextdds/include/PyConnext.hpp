...
template <typename T> void process_inits(py::module &, ClassInitList &);
...
void add_conversion(py_to_class &cls,
...
template <typename T> bool has_value(const T &opt) {
...
template <template <typename> class U, typename T> T &get_value(U<T> &opt) {
#if rti_connext_version_gte(6, 0, 1)
  return opt.value();
#else
  return opt.get();
...
namespace pybind11 {
namespace detail {
template <typename T> struct type_caster<dds::core::optional<T *>> {
  using base = type_caster_base<T>;

public:
  PYBIND11_TYPE_CASTER(dds::core::optional<T *>,
  ...
  static handle cast(const dds::core::optional<T *> &src, return_value_policy r,
                     handle h) {
...
namespace pybind11 {
namespace detail {
template <typename T> struct type_caster<rti::core::optional_value<T>> {
  using base = type_caster<T>;

public:
  PYBIND11_TYPE_CASTER(rti::core::optional_value<T>,
                       _("Optional[") + base::name + _("]"));
  ...
  static handle cast(const rti::core::optional_value<T> &src,
                     return_value_policy r, handle h) {
    if (pyrti::has_value(src))
      return base::cast(pyrti::get_value(src), r, h);
    return py::none().release();
  }
